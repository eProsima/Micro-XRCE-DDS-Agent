...
private:
     ...
     *                              included in the callback database.
                ...
                std::function<void (Args ...)>&& callback_function)
        {
            switch (callback_kind)
            {
                ...
                {
                    on_create_datawriter_callbacks_.emplace_back(Callback<Args ...>(std::move(callback_function)));
                    break;
                }
                case CallbackKind::CREATE_DATAREADER:
                {
                    on_create_datareader_callbacks_.emplace_back(Callback<Args ...>(std::move(callback_function)));
                    break;
                }
                ...
                const CallbackKind& callback_kind ...) const
        {
            ...
            switch (callback_kind)
            {
                ...
                case CallbackKind::CREATE_DATAWRITER:
                {
                    ...
                    {
                        on_create_datawriter_callback(guid, participant);
                    }
                    break;
                }
                case CallbackKind::CREATE_DATAREADER:
                {
                    ...
                    {
                        on_create_datareader_callback(guid, participant);
                    }
                    break;
                }
                case CallbackKind::CREATE_REQUESTER:
        ...
        using GenericFastEntityCallback = Callback<const fastrtps::rtps::GUID_t&,
                                                    const fastdds::dds::DomainParticipant*>;
        using CreateParticipantCallback = GenericFastEntityCallback;
        using DeleteParticipantCallback = GenericFastEntityCallback;
        using CreateDataWriterCallback = GenericFastEntityCallback;
        using CreateDataReaderCallback = GenericFastEntityCallback;
     ...
     **********************************************************************************************************************/
    class FastDDSCallbackFactory : public FastCallbackFactory
    {
    public:
        /**
         * @brief   Default constructor.
 ...
 **********************************************************************************************************************/
template <>
inline void CallbackFactory::FastCallbackFactory::add_callback<const FastDDSRequester&,
    const fastdds::dds::DomainParticipant*>(
        const CallbackKind& callback_kind,
...
template <>
inline void CallbackFactory::FastCallbackFactory::add_callback<const FastDDSReplier&,
    const fastdds::dds::DomainParticipant*>(
        const CallbackKind& callback_kind,
