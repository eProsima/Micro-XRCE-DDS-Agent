        ...
        const dds::xrce::OBJK_PARTICIPANT_Representation& representation)
{
    ...
    switch (representation.representation()._d())
    {
        case dds::xrce::REPRESENTATION_BY_REFERENCE:
        {
            ...
            created_entity = proxy_client->get_middleware().create_participant_by_ref(raw_object_id,
                            representation.domain_id(), ref_rep);
            break;
        }
        case dds::xrce::REPRESENTATION_AS_XML_STRING:
        {
            const std::string& xml_rep = representation.representation().xml_string_representation();
            created_entity = proxy_client->get_middleware().create_participant_by_xml(raw_object_id,
                            representation.domain_id(), xml_rep);
            break;
        }
        ...
        const dds::xrce::ObjectVariant& new_object_rep) const
{
    ...
    switch (new_object_rep.participant().representation()._d())
    {
        ...
        case dds::xrce::REPRESENTATION_IN_BINARY:
        {
            ...
            rv = proxy_client_->get_middleware().matched_participant_from_bin(get_raw_id(), domain_id,
                            participant_xrce);
            break;
        }
        default:
