...
bool DiscoveryServerWindows<EndPoint>::get_interfaces()
{
    bool rv = false;
        ...
        while (current_addr)
        {
            ...
            while (unicast_addr)
            {
                if (AF_INET == unicast_addr->Address.lpSockaddr->sa_family)
                {
                    ...
                    medium_locator.port(DiscoveryServer<EndPoint>::agent_port_);
                    medium_locator.address({uint8_t(unicast_addr->Address.lpSockaddr->sa_data[2]),
        ...
        while (current_addr)
        {
            ...
            while (unicast_addr)
            {
                if (AF_INET6 == unicast_addr->Address.lpSockaddr->sa_family)
                {
                    ...
                    large_locator.port(DiscoveryServer<EndPoint>::agent_port_);
                    struct sockaddr_in6* addr = reinterpret_cast<sockaddr_in6*>(unicast_addr->Address.lpSockaddr);
                    large_locator.address({addr->sin6_addr.s6_addr[0],
